...
#   File                :   flink_avg1.py
#
#   Description         :   building sort of the following, aka source from Kafka, sink to Fluss, but our payload, 
#                       :   flattened, and some averages, min, max etc calculated pertumbling window.
#               
#   Description         :   Building sort of the following, aka source IoT documents from a Python data generator, publish to Kafka, Source via
#                       :   Flink connector to Kafka, We now use a User Defined Python Job, to create a 1 minute tumbling window
#                       :   to determine the min, avg and max measurements per sensor, in addition to sensor readings per window.
#                       :   We lastly sink our output records with a window_start and window_end into a Fluss backed table.
#
...
def main(site_id_filter: int, input_kafka_topic: str, bootstrap_servers: str):
    ...
    pipeline_name = f"Flink_avg1-{input_kafka_topic}-{site_id_filter}"
    ...
    # --------------------------------------------------------------------------
    aggregation_sql = f"""
        INSERT INTO fluss_catalog.fluss.factory_iot_avg
            (siteId, deviceId, sensorId, partition_month, measurement_count, min_measurement, avg_measurement, max_measurement, window_start, window_end)
            SELECT
    ...
    print(f"PyFlink job finished for siteId: {site_id_filter}, flatten and avg from topic: {input_kafka_topic}, outputting to fluss")
