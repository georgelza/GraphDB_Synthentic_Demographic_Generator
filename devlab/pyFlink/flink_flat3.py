...
from complex_udf import join_string_udf

DEFAULT_BOOTSTRAP = 'broker:29092'


# --------------------------------------------------------------------------
# In this version we going to implement a very simply inline User Defined Function that will join the siteId and deviceId and output a string.
...
def main(site_id_filter: int, input_kafka_topic: str, bootstrap_servers: str):
    ...
    t_env.create_temporary_system_function(
        "join_udf",         # Name for the scalar UDF in SQL
        join_string_udf     # Reference the imported function directly
    )
    ...
    if site_id_filter == 101 or site_id_filter == 104:              # North
        flat_sql = f"""
            INSERT INTO fluss_catalog.fluss.factory_iot_complex
            (ts, siteId, deviceId, sensorId, unit, complex, ts_human, latitude, longitude, deviceType, measurement, partition_month)
                SELECT
                     ts                                             AS ts
                    ...
                    ,join_udf(metadata.siteId, metadata.deviceId)   AS complex -- Apply UDF here
