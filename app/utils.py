...
def getConfigs():
    ...
    config_params["ADULTS_TARGET"]          = int(os.environ["ADULTS_TARGET"])
    
    config_params["CHILDREN_STORE"]         = os.environ["CHILDREN_STORE"] 
    config_params["CHILDREN_TARGET"]        = int(os.environ["CHILDREN_TARGET"]) 
    
    config_params["FAMILY_STORE"]           = os.environ["FAMILY_STORE"] 
    config_params["FAMILY_TARGET"]          = int(os.environ["FAMILY_TARGET"])
    ...
    if 2 in config_params["TARGETS"]:
        config_params["POSTGRES_HOST"]              = os.environ["POSTGRES_CDC_HOST"]
        config_params["POSTGRES_PORT"]              = str(os.environ["POSTGRES_CDC_PORT"])
        config_params["POSTGRES_USER"]              = os.environ["POSTGRES_CDC_USER"]
        config_params["POSTGRES_PASSWORD"]          = os.environ["POSTGRES_CDC_PASSWORD"]
        config_params["POSTGRES_DB"]                = os.environ["POSTGRES_CDC_DB"]
...
def echo_config(config_params, mylogger):
    ...
    targets = ["=> MongoDB", "=> PostgreSql", "=> Redis", "=> Kafka"]
    if config_params["ECHOCONFIG"] == 1:
        ...
        mylogger.info("* Adult Target Specified           : " + targets[config_params["ADULTS_TARGET"] - 1])
        mylogger.info("* Children Store                   : " + config_params["CHILDREN_STORE"])
        mylogger.info("* Children Target Specified        : " + targets[config_params["CHILDREN_TARGET"] - 1])
        mylogger.info("* Families Store                   : " + config_params["FAMILY_STORE"])
        mylogger.info("* Families Target Specified        : " + targets[config_params["FAMILY_TARGET"] - 1])

        if 1 in config_params["TARGETS"]:  
            mylogger.info("* ")
            mylogger.info("* Target                           : 1")
