...
#   
#   Created         :   06 Aug 2025
#                   :   18 Aug 2025 - Added Kafka as destination
#
#   Classes         :   DatabaseConnectionError
...
class MongoDBConnection(DatabaseConnection):
    ...
    def connect(self) -> bool:
        ...
        self.mylogger.debug('MongoDB -> Configuring:')
        ...
        try:
            ...
            self.mylogger.info('MongoDB connection established to: {host}'.format(
                host    = self.config_params["MONGO_DATASTORE"]
            ))
            return True
...
class PostgreSQLConnection(DatabaseConnection):
    ...
    def connect(self) -> bool:
        ...
        self.mylogger.debug('PostgreSQL -> Configuring:')
        try:
            ...
            self._is_connected = True
            self.mylogger.info('PostgreSQL connection established to: {host} {db}'.format(
                host    = self.config_params["POSTGRES_HOST"],
                db      = self.config_params["POSTGRES_DB"]
            ))
            return True
            
        except PostgreSQLError as err:
            self.mylogger.error('PostgreSQL connection failed: {host}/{db} {err}'.format(
                host    = self.config_params["POSTGRES_HOST"],
                db      = self.config_params["POSTGRES_DB"],
                err     = err
            ))
    ...
    def insert_single(self, 
        ...
        Args:
            ...
            nationalid:     Unique identifier (optional)
        ...
        try:
            with self.get_cursor() as cursor:
                ...
                if nationalId is not None:
                    query = sql.SQL("""
                        INSERT INTO public.{store_name} (nationalid, data) 
                        VALUES (%s, %s)
                        RETURNING id, created_at;
                    """).format(store_name=sql.Identifier(store_name))
                    
                    cursor.execute(query, (nationalId, Json(json_string)))
    ...
    def insert_multiple(self, 
        ...
        Args:
            ...
            nationalid:     Whether to extract nationalid from JSON data
        """
        ...
        try:
            ...
            with self.get_cursor() as cursor:
                ...
                if extract_nationalId:
                    query = sql.SQL("""
                        INSERT INTO {store_name} (nationalid, data)
                        SELECT (jsonb_array_elements(%s)->>'nationalid'), jsonb_array_elements(%s)
...
class RedisConnection(DatabaseConnection):
    ...
    def connect(self) -> bool:
        ...
        self.mylogger.debug('Redis -> Configuring:')
        try:
...
class KafkaConnection(DatabaseConnection):
    ...
    def _reconnect_with_retry(self) -> bool:
        ...
        for i in range(self.max_retries):
            self.mylogger.debug("Attempting Kafka connection ({max_retries})...".format(
    ...
    def connect(self) -> bool:
        ...
        self.mylogger.debug('Kafka -> Configuring:')
...
class DatabaseManager:
    ...
    def connect(db_type:       str, 
        ...
        if db_type.lower() == 'mongodb':
            connection =  MongoDBConnection(config_params,    mylogger)
        
        elif db_type.lower() == 'postgresql':
            connection =  PostgreSQLConnection(config_params, mylogger)
        
        elif db_type.lower() == 'redis':
            connection =  RedisConnection(config_params,      mylogger)
        
        elif db_type.lower() == 'kafka':
            connection =  KafkaConnection(config_params,      mylogger)
...
def connect_to_kafka(config_params, 
                     mylogger):
    
    """Legacy function wrapper for Kafka connection"""
    
    kafka_conn = KafkaConnection(config_params, mylogger)
    kafka_conn.connect()
    
    return kafka_conn.client
#end connect_to_kafka
