...
def createBankAccount(fake, init, surname):
    ...
    while x < NumberOfAccounts:
        ...
        AgentAccountId, accountId   = fake.unique.accountNumber(bank_record["fspiId"])
        account = {
            "fspiAgentAccountId":   AgentAccountId,
            "accountId":            accountId,
            "fspiId":               bank_record["fspiId"],
            "fspiAgentId":          bank_record["fspiAgentId"],
            "accountType":          WeightedRandomSelector(accountTypes_options, scale=1).get_random(),
            "memberName":           bank_record["memberName"]
        }
...
def createCCAccount(fake, init, surname, arAccounts):
    ...
    while x < NumberOfCC:
        ...
        if cardNetwork == "Mastercard":    # Generate a Mastercard number
            ccNumber = fake.unique.credit_card_number(card_type='mastercard')

        elif cardNetwork == "Visa":         # Generate a Visa number
            ccNumber = fake.unique.credit_card_number(card_type='visa')

        elif cardNetwork == "Amex":         # Generate an American Express number
            ccNumber = fake.unique.credit_card_number(card_type='amex')

        elif cardNetwork == "Jcb":         # Generate an American Express number
            ccNumber = fake.unique.credit_card_number(card_type='jcb')
        
        elif cardNetwork == "Diners":         # Generate an American Express number
            ccNumber = fake.unique.credit_card_number(card_type='diners')
        
        card = {
            "cardHolder":   f"{init} {surname}",
            "cardNumber":   ccNumber,
            "expDate":      fake.exp_date(),
